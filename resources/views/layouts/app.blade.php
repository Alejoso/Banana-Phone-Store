...
<head>
    ...
    <title>@yield('title', __('messages.appTitle'))</title>
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark py-4">
    <div class="container">
        ...
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarNavAltMarkup"
            aria-controls="navbarNavAltMarkup" aria-expanded="false" aria-label="{{ __('messages.toggleNavigationTitle') }}">
            <span class="navbar-toggler-icon"></span>
        </button>

        <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
            <div class="navbar-nav ms-auto align-items-lg-center gap-lg-2">
                <a class="nav-link active" href="{{ route('home.index') }}">{{ __('messages.homeNavTitle') }}</a>
                <a class="nav-link active" href="{{ route('office.index') }}">{{ __('messages.viewOfficesButton') }}</a>
                <a class="nav-link active" href="{{ route('phone.index') }}">{{ __('messages.viewPhonesButton') }}</a>
                ...
                @if (auth()->check())
                    ...
                    <a class="nav-link active" href="{{ route('user.show' , auth()->user()->getId()) }}">{{ __('messages.manageAccountButton') }}</a>
                @endif

                @if (!auth()->check())
                    <a class="nav-link active" href="{{ route('login') }}">{{ __('messages.loginButton') }}</a>
                @endif
